        ...
        controller: function(storageUpdater) {
            ...
            function addItem() {
                ...
                if (vm.listIncasObj.length === 0) {
                    ...
                    delete itemCollect.$$hashKey;
                    itemCollect.id = vm.listIncasObj[vm.listIncasObj.length - 1].id + 1;
                }


                vm.listIncasObjAll.push(itemCollect);
            ...
            function deleteItem(id) {
                vm.listIncasObjAll.splice(id, 1);
                //
                storageUpdater.updateItem("collectItems", vm.listIncasObjAll);
            ...
            //выводить будем только те, которые создал пользователь
            function getItemsPersonal() {
                var person = storageUpdater.getItem("person");
                vm.listIncasObjAll = storageUpdater.getItem("collectItems");
                vm.listIncasObj = [];
                for (var i = 0; i < vm.listIncasObjAll.length; i++) {
                    if (vm.listIncasObjAll[i].idPerson === person.id) {

                        vm.listIncasObj.push(vm.listIncasObjAll[i]);
                    }
                }
            }
